...
const PrivacyPolicies = (props) => {
  ...
  useEffect(() => {
    ...
    if(from==="booking"){
            dropdownRef.current.alertWithType('Notice', t('sorry'), t('acceptPrivacyPolicies'));
        }
  }, []);
...
function extractDate(timestamp) {
  ...
  // Format the date as per your requirement, for example: YYYY-MM-DD
            ...
            <View>
             ...
             <View>
                {policydata.status === 1 &&(
                 <TouchableOpacity onPress={toggleDetails}>
                   {showDetails ? (
                             <Text style={{color:colors.medics_grey}}>▲</Text> // Arrow pointing up when details are shown
                           ) : (
                             <Text style={{color:colors.medics_grey}}>▼</Text> // Arrow pointing down when details are hidden
                           )}
                 </TouchableOpacity>
                 )}
             </View>
        ...
        <View style={{ position: 'absolute', bottom: 5, width: '100%', height: 100, flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
            {
              policydata.status === 0 ? ( // If policy status is 1
                ...
                <View style={{ width: '90%', backgroundColor: 'gray', borderRadius: 10, height: 50, alignItems: 'center', justifyContent: 'center' }}>
                  <Text style={{ color: 'white', fontSize: f_m, fontWeight: 'bold' }}>{t('acceptedAt')} {extractDate(policydata.accepted_at)} {t('accepted_At')}</Text>
                </View>
              )
            }
