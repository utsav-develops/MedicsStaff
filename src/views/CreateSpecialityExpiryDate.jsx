...
import { normal, bold, regular, f_xl, f_xs, f_m, expiry_icon } from '../config/Constants';
import Icon, { Icons } from '../components/Icons';
import DropdownAlert from 'react-native-dropdownalert';
import { connect } from 'react-redux';
import { updateSpecialityName } from '../actions/SpecialityDetailActions';
import DateTimePicker from '@react-native-community/datetimepicker';
import { useLocalization } from '../config/LocalizationContext';
import Translations from '../config/Translations';
import { useCustomTheme } from  '../config/useCustomTheme';

const CreateSpecialityExpiryDate = (props) => {
    const navigation = useNavigation();
    const [speciality_name, setSpecialityName] = useState('');
    ...
    const check_valid = () => {
        if (speciality_name) {
            ...
            dropDownAlertRef.alertWithType('error',t('validationError'), t('pleaseDBSexpiry'));
        }
    }
    ...
    const onDateChange = (event, selectedDate) => {
        ...
        setSpecialityName(formattedDate);
    };
    ...
    const navigate = async () => {
        Keyboard.dismiss()
        props.updateSpecialityName(speciality_name);
        go_back();
    }
     ...
     <>
        ...
        <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
            ...
            <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: f_xl, fontWeight: 'bold' }}>{t('dbsCertificateExpiry')}</Text>
                <View style={{ margin: 5 }} />
                <Text numberOfLines={1} style={{ color: colors.grey, fontSize: f_xs, fontWeight: 'normal' }}>{t('selectExpiryDate')}</Text>
                <View style={{ margin: 20 }} />
                <View style={{ width: '80%' }}>
...
const mapDispatchToProps = (dispatch) => ({
    updateSpecialityName: (data) => dispatch(updateSpecialityName(data)),
});

export default connect(null, mapDispatchToProps)(CreateSpecialityExpiryDate);