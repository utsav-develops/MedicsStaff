...
import { updateReferralCode } from '../actions/RegisterActions';
import { useLocalization } from '../config/LocalizationContext';
import Translations from '../config/Translations';
import { useCustomTheme } from  '../config/useCustomTheme';

const CreateReferralCode = (props) => {
  ...
  const { isDarkMode, toggleTheme, colors } = useCustomTheme();
    ...
    const check_valid = async () => {
       if(referral_code==''){
            ...
            props.updateReferralCode(referral_code);
        }
...
const styles = StyleSheet.create({
    ...
    <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
            ...
            <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: f_xl, fontWeight: 'bold' }}>{t('enterReferralcode')}</Text>
                <View style={{ margin: 5 }} />
                <Text numberOfLines={1} style={{ color: colors.grey, fontSize: f_xs, fontWeight: 'normal' }}>{t('ifReferralCode')}</Text>
                <View style={{ margin: 20 }} />
                <View style={{ width: '80%' }}>
...
const mapDispatchToProps = (dispatch) => ({
    updateReferralCode: (data) => dispatch(updateReferralCode(data)),
});


export default connect(null, mapDispatchToProps)(CreateReferralCode);