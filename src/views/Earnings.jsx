...
const Earnings = (props) => {
  ...
  const show_list = ({ item }) => (
    <View style={{ flexDirection:'row', width:'100%', marginBottom:20}}>
      ...
      <View style={{ width:'55%', justifyContent:'center'}}>
        <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: f_s, fontWeight: 'normal' }}>#{item.work_id}</Text>
        <View style={{ margin:2 }} />
        <Text numberOfLines={1} style={{ color: colors.text_grey, fontSize: f_tiny, fontWeight: 'normal' }}>{Moment(item.created_at).format("hh:mm A")}</Text>
      </View>
  ...
  return (
    ...
    <SafeAreaView
      style={{flex: 1 ,width: '100%', height: '100%',backgroundColor: colors.theme}}
    > 
    <View style={[styles.header]}>
            ...
            <View activeOpacity={1} style={{ width: '85%', alignItems: 'flex-start', justifyContent: 'center' }}>
              <Text numberOfLines={1} ellipsizeMode='tail' style={{ color: colors.theme_fg_three, fontSize: f_xl, fontWeight: 'bold' }}>{t('earnings')}</Text>
            </View>
          </View>
          ...
          <DropShadow
            ...
            >
              <View style={{ backgroundColor:colors.theme_bg_three, width:'100%', padding:20, alignItems:'center', justifyContent:'center', borderRadius:10}}>
                <Text numberOfLines={1} style={{ color: colors.text_grey, fontSize: f_s, fontWeight: 'normal' }}>{t('totalEarnings')}</Text>
                <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: f_35, fontWeight: 'bold', margin:10 }}>{global.currency}{today_earning}</Text>
              </View>
              <View style={{ margin:10 }} />
              <View style={{ backgroundColor:colors.theme_bg_three, width:'100%', padding:20, justifyContent:'center', borderRadius:10}}>
                <Text numberOfLines={1} style={{ color: colors.text_grey, fontSize: f_s, fontWeight: 'normal' }}>{t('workHistories')}</Text>
                <View style={{ margin:10 }} />
                {loading == true ?
                  ...
                  <View>
                    ...
                    :
                      <View style={{ justifyContent:'center', alignItems:'center' }}>
                        ...
                        <View style={{ margin:10 }} />
                        <Text numberOfLines={1} style={{ color: colors.text_grey, fontSize: f_s, fontWeight: 'normal' }}>{t('sorryNoData')}</Text>
                      </View>
                    }
                  </View>
                }
