...
const Splash = (props) => {
    ...
    await axios({
      ...
      .then(async response => {
        if(response.data.result.android_latest_version.version_code > app_version_code){
          navigate_update_app('https://play.google.com/store/apps/details?id=com.medics.staff');
        }else{
  ...
  const home = async (data) => {
    ...
    global.staff_work_time = data.staff_work_time;
  ...
  const channel_create = () => {
    PushNotification.createChannel(
      {
        channelId: "staff_booking", // (required)
        channelName: "Booking", // (required)
        channelDescription: "Locum Solution", // (optional) default: undefined.
