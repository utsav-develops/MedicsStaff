...
const EditAccountNumber = (props) => {
    ...
    const check_valid = () => {
        if (account_number) {
            ...
            dropDownAlertRef.alertWithType('error', t('validationError'), t('pleaseEnterAccNum'));
        }
    }
    ...
    const call_update_kyc = () => {
        ...
        axios({
            ...
            data: { staff_id: global.id, account_number: account_number }
        })
            .then(async response => {
                setLoading(false);
                dropDownAlertRef.alertWithType('success', t('successfullyUpdated'), t('AccNumUpdated'));
                go_back();
            })
            .catch(error => {
                setLoading(false);
     ...
     <>
        ...
        <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
            ...
            <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: f_xl, fontWeight: 'bold' }}>{t('enterAccNum')}</Text>
                <View style={{ margin: 5 }} />
                <Text numberOfLines={1} style={{ color: colors.grey, fontSize: f_xs, fontWeight: 'normal' }}>{t('needEnterAccNum')}</Text>
                <View style={{ margin: 20 }} />
                <View style={{ width: '80%' }}>
