...
const EmergencyContacts = (props) => {
    ...
    const call_delete_sos_contact = (data) => {
        ...
        axios({
            ...
            data: { staff_id: global.id, contact_id: data.id }
        })
        .then(async response => {
            setLoading(false);
        ...
        return (
            <View style={styles.animationHeader}>
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    <TouchableOpacity style={{ width: '30%', alignItems: 'flex-start', justifyContent: 'center', height: 60, paddingLeft: 17, top: Platform.OS === "ios" ? 45 : 0, }}>
                        <Icon type={Icons.MaterialIcons} name="arrow-back" color={colors.theme_fg_three} style={{ fontSize: 30 }} />
                    </TouchableOpacity>
                </View>
                <View style={{ padding: 30, alignItems: 'center' }}>
                    <Text numberOfLines={2} ellipsizeMode='tail' style={{ color: colors.theme_fg_three, fontSize: f_30, letterSpacing: 1, fontWeight: 'bold', textAlign: 'center' }}>{t('setupEmergencyContact')}</Text>
                </View>
            </View>
        )
    }
        ...
        <View style={styles.container}>
            ...
            >
                ...
                <TouchableOpacity onPress={navigate_add_contact.bind(this)} style={{ justifyContent:'center', alignItems:'center', marginTop:'20%' }}>
                    ...
                    <Text numberOfLines={1} style={{ color: colors.text_grey, fontSize: f_s, fontWeight: 'normal' }}>Add your contact</Text>
                </TouchableOpacity>
                }
                {loading == true &&
            ...
            <Animated.View opacity={headerHeight} style={[styles.header]}>
                ...
                <TouchableOpacity activeOpacity={1} style={{ width: '83%', alignItems: 'flex-start', justifyContent: 'center' }}>
                    <Text numberOfLines={1} ellipsizeMode='tail' style={{ color: colors.theme_fg_three, fontSize: f_xl, fontWeight: 'bold' }}>{t('emergencyContact')}</Text>
                </TouchableOpacity>
            </Animated.View>
        </View>
    );
