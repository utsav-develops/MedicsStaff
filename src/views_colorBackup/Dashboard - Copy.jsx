...
const Dashboard = (props) => {
  ...
  const borderScaleAnim = useRef(new Animated.Value(1)).current; // Start with no scale
  const handleIdIconPress = () => {
    // Add logic to navigate to the ID screen or show a popup
    // Example: navigation.navigate('IDScreen');
    console.log('ID Icon pressed'); // Placeholder for your logic
  };


  const animateBackgroundColor = () => {
    Animated.timing(backgroundColorAnim, {
      toValue: switch_value ? 1 : 0, // 1 for "online", 0 for "offline"
      duration: 1000, // Duration of the animation
      useNativeDriver: false // This needs to be false to animate colors
    }).start();
  };
  ...
  useEffect(() => {
    // Define a function to call the heatmap coordinates logic
    const callGetHeatmapCoordinates = () => {
      // Your existing logic to get heatmap coordinates
    };

    // Set the intervals for heatmap coordinates and booking sync
    ...
    // Check the switch value and animate accordingly
    if (switch_value) {
      // Animate to show the user is online
      animateBackgroundColor(true);
    } else {
      // Animate to show the user is offline
    ...
    return () => {
      ...
      unsubscribeFocusListener(); // Correctly unsubscribe from the navigation focus listener
    };
  }, [switch_value]); // Add switch_value to the dependency array to re-run the effect when it changes
  ...
  const backgroundColorInterpolate = backgroundColorAnim.interpolate({
    inputRange: [0, 1],
    outputRange: ['rgba(255, 255, 255, 0)', colors.success], // Starting from transparent and ending with your color
  });
  ...
  return (
    <View style={styles.container} {...panResponder.panHandlers}>
      ...
      <View style={styles.topRightContainer}>
        <TouchableOpacity onPress={handleIdIconPress}>
...
<View style={styles.bottomContainer}>
      <Animated.View
        style={[
          styles.fab,
          {
            ...
            backgroundColor: switch_value ? colors.success : colors.theme_bg, // Change the color based on switch_value
          },
        ]}
      >
        <TouchableOpacity onPress={toggleSwitch}>
          <View style={{ alignItems: 'center' }}>
            {switch_value ? (
             <>
                <Text style={[styles.fabText, { color: '#FFFFFF' }]}>ONLINE</Text>
                <Text style={{ fontSize: 16, color: '#000' }}>Tap to go offline</Text>
              </>
            ) : (
              <>
      ...
      <Animated.View style={[styles.innerContainer, { backgroundColor: backgroundColorInterpolate }]}>
  {/* Here goes your content that should have the animated background color */}
  <View style={styles.horizontalLine} />
  <View style={styles.statusTextContainer}>
    {switch_value ? (
      <Text style={{ color: colors.online_text, fontSize: 20, fontWeight: 'bold' }}>You are online</Text>
    ) : (
...
topRightContainer: {
  position: 'absolute',
  top: Platform.OS === "android" ? StatusBar.currentHeight : 0, // adjust for the status bar height
  right: 10,
